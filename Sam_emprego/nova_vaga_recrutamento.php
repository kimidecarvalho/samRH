...
<head>
  ...
  <style>
    * {
    ...
    /* --- CSS das tags de competências (copiado do job_register_page.php) --- */
    .tag-container {
      display: flex;
      flex-wrap: wrap;
      gap: 8px;
    ...
    .tag {
      ...
      font-size: 0.95rem;
      font-weight: 500;
      box-shadow: 0 1px 2px rgba(44,163,120,0.07);
...
<body>
        ...
        <div class="form-group">
          ...
          <div class="form-row">
            <div class="form-column">
              ...
              <input type="hidden" name="competencias" id="competencias-input">
            </div>
            <div class="form-column">
              <input type="text" id="new-competencia" class="form-control" placeholder="Selecione ou digite..." list="competencias-list" onkeydown="if(event.key === 'Enter') { event.preventDefault(); addCompetencia(); }">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (expirationInput && expirationInput.value) {
                ...
                const expDate = new Date(expirationInput.value);
                if (expDate < today) {
                    const toast = document.getElementById('toast');
                    const toastMessage = document.getElementById('toast-message');
                    toast.style.backgroundColor = '#e74c3c';
                    toastMessage.textContent = 'A data de expiração não pode ser no passado.';
    ...
    // Lista de tags de competências
    let competenciasTags = [];

    // Função para adicionar uma competência
    function addCompetencia() {
      const input = document.getElementById('new-competencia');
      const tagText = input.value.trim();
      if (tagText && competenciasPermitidas.includes(tagText) && !competenciasTags.includes(tagText)) {
        competenciasTags.push(tagText);
        renderCompetencias();
        input.value = '';
        document.getElementById('competencias-input').value = competenciasTags.join(', ');
    ...
    // Função para remover uma competência
    function removeCompetencia(index) {
      competenciasTags.splice(index, 1);
      renderCompetencias();
      document.getElementById('competencias-input').value = competenciasTags.join(', ');
    }

    // Função para renderizar as competências
    function renderCompetencias() {
      const container = document.getElementById('tag-container');
      container.innerHTML = '';
      competenciasTags.forEach((tag, index) => {
        const tagElement = document.createElement('div');
        tagElement.className = 'tag';
        tagElement.innerHTML = `${tag} <span class="tag-remove" onclick="removeCompetencia(${index})">×</span>`;
